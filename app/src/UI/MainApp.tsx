...
function MainApp() {
    ...
    const allBugs = critterList.bugs.sort(sortCritterList).filter((critter: ICritter) => state.hideCaught ? state.caughtCritters.bugs.indexOf(critter.id) < 0 : critter );
    ...
    const allFish = critterList.fish.sort(sortCritterList).filter((critter: ICritter) => state.hideCaught ? state.caughtCritters.fish.indexOf(critter.id) < 0 : critter );
    ...
    return (
        ...
        : <div>
            ...
            <Checkbox label={"Show all critters"} isSelected={state.showAll} onCheckboxChange={() => store.dispatch(showAll(!state.showAll))} />
            {
                ...
                <ul>
                    {
                        availableBugs.map((critter:ICritter) => <CritterEntry typeOfCritter={critterType.bug} critter={critter} key={`available_bug_${critter.id}`} />)
                    }
                </ul>
                <h1>You can catch {upcomingCritters.bugs.length} Bugs later today:</h1>
                <ul>
                    {
                        upcomingBugs.map((critter:ICritter) => <CritterEntry typeOfCritter={critterType.bug} critter={critter} key={`donated_bug_${critter.id}`} />)
                    }
                </ul></>
            }
            {
                state.showAll ? <><h1>All Fish</h1>
                <ul>
                    {
                        allFish.map((critter:ICritter) => <CritterEntry typeOfCritter={critterType.fish} critter={critter} key={`all_fish_${critter.id}`} />)
                    }
                </ul></>
                : <><h1>You can currently catch {availableCritters.fish.length} Fish:</h1>
                <ul>
                    {
                        availableFish.map((critter:ICritter) => <CritterEntry typeOfCritter={critterType.fish} critter={critter} key={`available_fish_${critter.id}`} />)
                    }
                </ul>
                <h1>You can catch {upcomingCritters.fish.length} Fish later today:</h1>
                <ul>
                    {
                        upcomingFish.map((critter:ICritter) => <CritterEntry typeOfCritter={critterType.fish} critter={critter} key={`donated_fish_${critter.id}`} />)
                    }
                </ul></>
            }
        </div>
